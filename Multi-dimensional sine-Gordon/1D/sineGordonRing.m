...
tmax = 120;                 % last time of simulation
dt = 1e-2;                  % time step
...
%% Initial condition.
initialCondition = 'ring-shaped kink';
...
for e = 1:nElements
    ...
    for g = 1:nGaussPoints
        ...
        r_g = Mesh.Nodes(Mesh.Elements(:,e)) * A(:,:,g)';
        K(eDoF, eDoF) = K(eDoF, eDoF) ...
            + (B(:,:,gIndex)' - dim_red/r_g * A(:,:,g)')* B(:,:,gIndex) * WxJ(gIndex);
    end
end
M = sparse(M);
K = sparse(K);
...
activeDoF = 1:gDoF;
TOL = 1e-10;             % Tolerance for stopping the Newton-Raphson.
for it = 1:nTimeSteps
    t(it+1) = it * dt;
    ...
    kt = sparse(kt);
    kt = kt + K;            % Compute the computation of "kt".
    ...
    for iter = 1:maxIter        
        ...
        % Convergence test.
        residual = norm(R(activeDoF), 2) / sqrt(gDoF);
        if residual < TOL
